...
function monthGrid(y,m,showWeeks=true){const wd=['一','二','三','四','五','六','日'],first=getMon(new Date(y,m,1)),last=new Date(y,m+1,0),td=todayStr(),ed=new Set(getDates()),stats=getStats();let h='<div class="cal-wk">'+(showWeeks?'<span></span>':'');wd.forEach(w=>h+='<span>'+w+'</span>');h+='</div><div class="cal-days">';for(let rowStart=new Date(first);rowStart<=last||rowStart.getMonth()===m;rowStart=addD(rowStart,7)){h+='<div class="cal-week">';if(showWeeks)h+='<span class="cal-week-no" data-cal-action="select-week" data-date="'+ds(rowStart)+'" title="第 '+isoWeekNumber(rowStart)+' 周">W'+isoWeekNumber(rowStart)+'</span>';for(let i=0;i<7;i++){const d=addD(rowStart,i),s=ds(d),level=statLevel(stats[s]||0),sub=showWeeks?calendarSubText(d):{text:'',fest:false};let c='cal-d';if(d.getMonth()!==m)c+=' om';if(s===td)c+=' td';if(s===selectedDate)c+=' sel';let mark='';if(level)mark='<span class="dot i'+level+'"></span>';else if(ed.has(s))mark='<span class="dot"></span>';h+='<span class="'+c+'" data-cal-action="select-date" data-date="'+s+'"><span class="cal-main">'+d.getDate()+'</span>'+(sub.text?'<span class="cal-sub'+(sub.fest?' fest':'')+'">'+sub.text+'</span>':'')+mark+'</span>'}h+='</div>'}h+='</div>';return h}
